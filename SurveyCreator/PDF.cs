        ...
        void CreatePDFSurvey()
        {
            ...
            foreach (SORULAR item in ANKET.sorular)
            {
                ...
                if (item.SoruTipi == enSoruTipleri.KisaMetin)
                {
                    ...
                    if (ANKET.SoruSayisiniYaz)
                    {
                        paragraph.AddText(index.ToString() + ") ");
                    }

                    paragraph.AddText(item.SoruBasligi);
                    paragraph.Format.Font.Size = 10;
                    //paragraph.Format.Font.Color = Colors.LightGray;
                    paragraph.Format.Alignment = ParagraphAlignment.Left;

                    paragraph = section.AddParagraph();
                    paragraph.AddText(item.SoruAciklamasi);
                    paragraph.Format.Font.Size = 8;
                    paragraph.Format.Alignment = ParagraphAlignment.Left;

                    paragraph = section.AddParagraph();
                    paragraph.Format.Alignment = ParagraphAlignment.Center;
                    paragraph.Format.Borders.Bottom = new Border() { Width = "0.5pt", Color = Colors.Gold };
                }
